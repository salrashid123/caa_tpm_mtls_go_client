...
func main() {
	...
	fmt.Printf("======= reloading key from file ========")
	...
	fmt.Printf("======= reading key from file ========")
	...
	tcrt, err := signer.TLSCertificate()
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	//tp := NewTokenProviderFromSource(ts)
	tp := oauth2adapt.TokenProviderFromTokenSource(ts)

	// ce := grpccredentials.NewTLS(&tls.Config{
	...
	opts := &grpctransport.Options{
		Credentials: &auth.Credentials{
			TokenProvider: tp,
		},
		// have to set the endpoint here
		Endpoint: "storage.mtls.googleapis.com:443",
		ClientCertProvider: func(c *tls.CertificateRequestInfo) (*tls.Certificate, error) {
			return &tcrt, nil
		},
		//GRPCDialOpts: []grpc.DialOption{grpc.WithTransportCredentials(ce)},
	}

	ctx := context.Background()
	client, err := grpctransport.Dial(ctx, true, opts)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	storageClient, err := storage.NewGRPCClient(ctx,
		option.WithGRPCConn(client.Connection()))
